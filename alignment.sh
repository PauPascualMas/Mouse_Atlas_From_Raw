...
## ALIGNMENT: STARsolo (STAR 2.7.10a) ##

# STARsolo takes cDNA (R1) read and CB+UMI (R2+R3, which needs to be created) read to perform the alignment. 
...
## In our case, index is directly obtained from https://www.thepoollab.org/resources, as well as the optimized genome and reference annotation
...
#Select stage and sample to align (change it manually for every sample)
...
#creating fastq file merging CB and UMI (in 10x Chemistry v1 are in separate files), as required for the alignment
...
### Running STAR
# Parameters are set to resemble CellRanger results, as well as matching CB and UMI length for v1 Chemistry
# For other protocols, specifications are attached below with the required vell barcode whitelist too.
